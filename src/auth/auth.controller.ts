...
import { AUTH_SERVICE_NAME, RegisterResponse, LoginResponse, ValidateResponse, LogoutResponse, LogoutRequest, UpdateRequest, UpdateResponse, ChangePasswordRequest, ChangePasswordResponse, ResetPasswordRequest, ForgetPasswordResponse, AddWalletAmountRequest, AddWalletAmountResponse, CheckWalletRequest, CheckWalletResponse, TransferMoneyRequest, TransferMoneyResponse } from './auth.pb';
...
export class AuthController {
  ...
  private addWalletAmount(payload: AddWalletAmountRequest ): Promise<AddWalletAmountResponse> {
    return this.service.addWalletAmount(payload);
  }

  @GrpcMethod(AUTH_SERVICE_NAME, 'checkWallet')
  private checkWallet(payload: CheckWalletRequest): Promise<CheckWalletResponse> {
    return this.service.checkWallet(payload)
  }

  @GrpcMethod(AUTH_SERVICE_NAME,'moneyTransaction')
  private moneyTransaction(payload:TransferMoneyRequest): Promise<TransferMoneyResponse> {
